...
const WorkoutComponent = () => {
  ...
  const filteredExercises = exercisesList.filter(
    (exercise) =>
      !exercises.some((addedExercise) => addedExercise.id === exercise.id)
  );
  ...
  const addExercise = async () => {
    ...
    const response = await fetch(
      `http://localhost:5260/api/workouts/${workoutId}/exercises`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${jwt_token}`,
  ...
  const endWorkout = async () => {
    await fetch(`http://localhost:5260/api/workouts/${workoutId}`, {
      method: "PUT",
    });
            ...
            <div key={exercise.id} className="exercise">
              ...
              <div className="sets">
                {exercise.sets.map((set) => (
                  <div
                    key={set.id}
                    className={`set ${set.completed ? "completed" : ""}`}
                  >
            ...
            previousWorkouts.map((workout) => (
              <li key={workout.id}>
                <div onClick={() => toggleWorkoutExpansion(workout.id)}>
                  <strong>{workout.name}</strong> ({workout.type}) -{" "}
                  {workout.duration} minutes (
                  {workout.workoutExerciseOrders &&
                    workout.workoutExerciseOrders.length}{" "}
                  Exercises)
                  {workout.description && (
                      ...
                      {workout.workoutExerciseOrders.map((weo) => (
                        <li key={weo.id}>
                          ...
                          <ul>
                            {workout.sets
                              .filter(
                                (set) => set.exerciseId === weo.exercise.id
                              )
                              .map((set) => (
                                <li key={set.id}>
                ...
                <button onClick={() => openModal(workout)}>Schedule</button>
              </li>
            ))
          ) : (
